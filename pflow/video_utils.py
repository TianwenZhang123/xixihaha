...
def _load_video_decord(
    ...
    # Convert to tensor (C, F, H, W) float [0, 1]
...
def save_video_tensor(
    ...
    """
    if video.dim() == 5:
        video = video[0]  # Take first batch element
...
def resize_video(
    ...
    # Reshape to (C*F, 1, H, W) for batch interpolation, then reshape back
...
def create_composite_video(
    ...
    """
    Create a side-by-side composite video from multiple video tensors.
    
    Used for VLM comparison: [V_ref | V_gen] or [V_ref | V_prev | V_curr]
    ...
    Args:
        ...
        label_height: Height reserved for labels (pixels).
        
    Returns:
        Composite video tensor (C, F, H_composite, W_composite).
    """
    # Ensure all videos have the same number of frames and height
    num_frames = min(v.shape[1] for v in videos)
    height = videos[0].shape[2]
    
    # Resize all videos to same height if needed
    processed_videos = []
    for v in videos:
...
def normalize_video(video: torch.Tensor) -> torch.Tensor:
    """
    Normalize video tensor to [-1, 1] range.
    
    Args:
        video: Video tensor in [0, 1] range.
