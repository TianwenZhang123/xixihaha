...
| α (alpha) | 0.001 | 噪声混合权重 (Eq. 10) |
| ρ_s | 0.1 | 空间SVD比例: 去除前10%空间分量 |
| ρ_m | 0.9 | 时序SVD比例: 保留前90%时序动态 |
...
参考视频 V_ref
    ...
    → Flow Matching Inversion → 反演噪声 η_inv
    → SVD Spatial Filter (去除前10%空间分量，消除具体内容)
    → SVD Temporal Retain (保留前90%时序分量，保留运动结构)
    → 与新随机噪声混合: η = √0.001·η_temporal + √0.999·η_new
    → 得到增强噪声先验 enhanced_noise
```

SVD 滤波的核心目的：从参考视频中提取"运动骨架"（时序结构），去掉具体的"内容外观"（空间特征），让生成模型在保持类似运动模式的同时自由发挥内容。

### 阶段二：Test-Time Prompt Optimization（固定3轮迭代）
...
输入给 Wan 2.1 的内容：
  - prompt: 当前迭代的文本提示词
  - latents: enhanced_noise (SVD增强噪声，三轮共享)
...
4. **调整 Noise Prior 参数**：
   - 增大 α (如 0.01) 可让更多运动信息传递到生成中
   - 调整 ρ_s/ρ_m 可改变空间/时序信息的保留程度
