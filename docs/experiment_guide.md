...
google-generativeai>=0.8.0 # Gemini VLM API
scipy>=1.11.0             # SVD 计算
imageio>=2.31.0           # 视频 I/O
decord>=0.6.0             # 高性能视频解码
```
...
| 服务 | 用途 | 获取方式 |
|------|------|---------|
| Google Gemini API | VLM prompt 优化 (P-Flow & VISTA) | https://aistudio.google.com/apikey |
...
# 方式三: 命令行传入（每次运行时指定）
python scripts/run_pflow.py --vlm_api_key "your-key" ...
```
