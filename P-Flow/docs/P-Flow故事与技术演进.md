...
**Layer 3 — Feature Injection with Confidence-Weighted Gating（特征注入与置信度加权门控）**
...
其中 $\lambda^{\ell}_{\text{eff}}$ 由三重门控确定：

$$\lambda^{\ell}_{\text{eff}} = \lambda_t \cdot \sigma(5 \cdot \bar{c}) \cdot g(h^{\ell}_{\text{current}}, h^{\ell}_{\text{ref}})$$

- $\lambda_t = \lambda_{\max} \cdot \sin(\pi t / (N-1))$：middle-peak 调度，中间阶段最强注入
- $\sigma(5 \cdot \bar{c})$：quality scale，来自 $\eta_{\text{temporal}}$ 帧间余弦相似度的软门控
- $g(h_c, h_r) = 1 - \text{sigmoid}(\tau \cdot (\cos(h_c, h_r) - 0.5))$：自适应特征对齐门控
