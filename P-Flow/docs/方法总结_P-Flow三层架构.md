...
$$
\text{gate} = 1 - \sigma(\tau \cdot (\text{sim} - 0.5)) \tag{28}
$$

$$
\lambda_{\text{eff}} = \lambda(t) \cdot \text{gate} \cdot \text{QS} \tag{29}
$$

其中 $\tau = 5.0$ 为温度参数，$\text{QS}$ 为质量门控分数。

**门控逻辑**：

- 当 $\text{sim}$ 高（特征已对齐）→ $\text{gate} \approx 0$ → 不注入（无需引导）
- 当 $\text{sim}$ 低（特征偏离）→ $\text{gate} \approx 1$ → 强注入（需要纠正方向）

### 4.6 Quality Scale（质量门控）
...
$$
\text{QS} = 0.1 + 0.9 \cdot \sigma(20 \cdot (\text{mean\_cos} - \theta)) \tag{31}
$$

其中 $\theta = 0.05$ 为阈值。当 $\text{mean\_cos} \gg \theta$（运动一致性高）时 $\text{QS} \approx 1.0$；当 $\text{mean\_cos} < \theta$（帧间无连贯运动）时 $\text{QS} \approx 0.1$，大幅压制注入。
...
$$
$$
\text{gate} = 1 - \sigma(\tau \cdot (\text{sim} - 0.5))
$$
$$
\lambda_{\text{eff}} = \lambda_{\max} \cdot \sin\left(\frac{\pi t}{N-1}\right) \cdot \text{gate} \cdot \text{QS}
$$
$$
...
| L2+L3 (SVD + FI adaptive) | **0.9042** | **0.8138** | **+3.3% / +8.6%** |
...
| `α` (alpha) | 0.004 | SVD temporal 混合系数 | 🔴 极高 |
...
3. **自适应 Feature Injection**通过余弦门控和 Quality Scale 实现动态强度调节，避免过度约束
4. **三层正交设计**消除层间冲突，实现超越单层贡献之和的协同效果

### 9.2 当前局限

1. 场景型视频（镜头运动、FPV 穿越）的 SVD 增益不稳定甚至为负，TSR 自适应尚未完全解决
2. 全局统一的 α/λ 对不同场景类型次优，per-sample 自适应策略有待验证
3. FI 的 Quality Scale 依赖 $\eta_{\text{temporal}}$ 帧间余弦，该指标与实际注入效果的相关性有限

### 9.3 未来方向

1. **场景感知的注入策略**：基于视频类型分类（物体运动 vs 镜头运动 vs 静态场景）自适应选择 α、β、λ 和目标层
2. **方向对齐度指标**：设计 prompt-motion cosine similarity 指标，在注入前判断 η_temporal 方向是否与 prompt 运动语义一致
...
*文档版本: v1.0 | 更新日期: 2026-06-19 | 基于 P-Flow 30 样本实验数据*
