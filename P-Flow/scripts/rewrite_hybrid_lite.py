#!/usr/bin/env python3
"""
Hybrid Prompt 改写脚本 v7e-lite — 外观优化 + 运动保留（为 SVD 留空间）

设计理念：
  v7e 通过 temporal chain + motion fidelity 使 prompt 精确编码运动信息，
  导致 SVD 先验与 prompt 信息冗余/冲突。v7e-lite 的核心改动是：
  - 保留 v7e 中对 CLIP 贡献最大的外观优化（subject-first, visual inference, camera closing）
  - 对运动描述完全不做修改，逐字保留 VLM 原文的模糊运动表述
  - 这样 SVD 先验可以补充"怎么动"的方向信息，形成信息互补而非冲突

与 v7e 的区别：
  v7e:      4 处修改 = opening + temporal chain + visual inference + camera closing
  v7e-lite: 3 处修改 = opening + visual inference + camera closing（去掉 temporal chain）

与 v8 的区别：
  v8:       删除所有运动描述，只保留外观 → 灾难性退化（-11.5% CLIP）
  v7e-lite: 保留运动描述原文不变 → 维持 VLM 原始运动信息的完整性

预期效果：
  - CLIP 接近 v7e（外观优化完整保留）
  - 与 SVD v1 组合后 X-CLIP 有正向增益（运动信息由 SVD 补充而非冲突）

用法:
    python scripts/rewrite_hybrid_lite.py \
        --input-dir /path/to/baseline_captions \
        --output-dir /path/to/hybrid_lite_captions \
        --backend dashscope \
        --model qwen-plus
...
# ─────────────────────────────────────────────────────────────────────────────
# System Prompt v7e-lite: 外观优化 + 运动描述完全保留
# ─────────────────────────────────────────────────────────────────────────────

SYSTEM_PROMPT = """You restructure VLM video captions into better T2V generation prompts for the Wan2.1 model. Your output must be nearly identical to the input — you make only 3 surgical changes and copy everything else VERBATIM.
...
2. ONE VISUAL INFERENCE — Add exactly ONE material/texture/lighting attribute that is physically implied (e.g., "wooden hulls" → "dark brown hulls"; "dim light" → add "silhouette effect"). This must be a surface-level property — NEVER a new object, person, animal, action, or weather phenomenon.

3. CAMERA CLOSING — End with a brief camera note (e.g., "The camera remains steady in close-up" / "The camera follows smoothly"). This anchors the UMT5 tail-token weight.

## MOTION PRESERVATION (HIGHEST PRIORITY — overrides all other rules)

ALL motion/action descriptions from the input MUST be copied VERBATIM with ZERO changes. You must NOT:
- Add temporal connectors (initially/then/gradually/as the scene progresses) to motion sentences
- Change motion verbs in any way (keep exact original wording)
- Change motion speed or direction words
- Add trajectories, directions, or motion patterns not in the input
- Rephrase or restructure sentences that describe motion/movement/action
- Split or merge motion sentences
- Add temporal decomposition to actions (e.g., "initially... then... finally...")

If the input says "The boat moves on the water", your output must contain "The boat moves on the water" — NOT "The boat initially drifts forward, then glides across the water".

Motion sentences must appear in the SAME ORDER and with the SAME WORDING as the input.
...
- Do NOT add temporal markers to ANY sentences
...
3. Make exactly 3 modifications: opening, one visual inference, camera closing
4. Verify: ALL motion sentences are UNCHANGED from input, only structural opening/ending differs
...
OUTPUT: "Two small sailboats with white sails and dark brown hulls floating on a cup of dark coffee. They appear to be miniature models. The liquid in the cup is smooth, with some ripples around the boats, suggesting a gentle movement. The lighting highlights the reflective surface of the liquid, creating subtle reflections of the boats. The background is slightly blurred, focusing attention on the cup and the boats. The overall scene has a serene and whimsical feel, as if the boats are sailing on a miniature sea within the cup. The camera remains steady in close-up throughout."
WHY: Subject="Two small sailboats" moved to front. Visual inference="dark brown hulls" (wooden→dark brown). Camera closing added. Motion sentences ("smooth, with some ripples...suggesting a gentle movement") UNCHANGED.
...
OUTPUT: "White SUV with roof racks driving on a dusty unpaved road through a forested area. The vehicle is equipped with roof racks carrying luggage or gear, suggesting it might be on a journey or adventure. As the SUV moves forward, it kicks up a cloud of dust behind it, indicating the dryness of the terrain and the speed at which it is traveling. The surrounding environment features tall pine trees and a scenic view of distant mountains under a clear blue sky. The overall atmosphere conveys a sense of exploration and outdoor adventure. The camera follows the vehicle smoothly along the rugged terrain."
WHY: Subject="White SUV" moved to front. Visual inference="dense vegetation implied by forested" (minor). Camera closing added. Motion sentence ("As the SUV moves forward, it kicks up a cloud of dust...") UNCHANGED.
...
OUTPUT: "Two adorable golden retriever puppies with fluffy golden fur playing joyfully in a snowy landscape. The scene is set during what appears to be late afternoon, as indicated by the warm, soft light casting long shadows on the snow. The puppies are energetically moving through the snow, their paws kicking up small clouds of snow as they play. Their tails are wagging, and their expressions convey a sense of excitement and happiness. The background shows a serene winter setting with snow-covered ground and bare trees, adding to the picturesque and cozy atmosphere. The camera remains steady, capturing the puppies' playful energy in the warm golden light."
WHY: Subject="Two adorable golden retriever puppies" at front. Visual inference=none needed. Camera closing added. ALL motion sentences ("energetically moving through the snow, their paws kicking up...") UNCHANGED.
...
OUTPUT: "Massive volcanic eruption set against lush green mountains and a body of water. The sequence begins with a large plume of dark smoke and ash rising into the sky, accompanied by bright blue lightning bolts that strike through the cloud. As the frames progress, the cloud of smoke becomes denser and more voluminous, expanding upwards and outwards. The surrounding landscape is bathed in dim light of early evening, adding to the ominous atmosphere of the scene. The ocean in the background remains calm, contrasting sharply with the intense activity of the volcano. The camera captures the full dramatic scale of the eruption against the towering volcanic column."
WHY: Subject="Massive volcanic eruption" at front. Visual inference="towering volcanic column" (expanding upwards implied). Camera closing added. ALL motion sentences ("rising into the sky", "becomes denser...expanding upwards and outwards") UNCHANGED.
...
OUTPUT: "Athletic person wearing a white tank top and black shorts running against a plain, light-colored background. The lighting is soft and even, casting minimal shadows and emphasizing the runner's movement. The person appears to be jogging at a steady pace, with their arms swinging naturally as they run. The overall atmosphere is focused on the physical activity and the simplicity of the setting. The camera remains stationary, capturing the runner's stride against the clean minimal backdrop."
WHY: Subject="Athletic person" at front. Visual inference=none needed (scene already specific). Camera closing added. Motion sentence ("jogging at a steady pace, with their arms swinging naturally") UNCHANGED.

## Output ONLY the restructured prompt. No explanations."""

USER_TEMPLATE = """Restructure this VLM caption ({word_count} words). First, identify what MOVES — that is your action subject. Then make ONLY 3 changes: (1) move action subject to first word, (2) one visual inference if needed, (3) add camera closing. Copy ALL other sentences VERBATIM — especially ALL motion/action descriptions must remain EXACTLY as written. Do not add temporal connectors. Output must be ~{word_count} words (±15%).
...
# ─────────────────────────────────────────────────────────────────────────────
# VLM 验证纠错 System Prompt（复用 v7e 版本）
# ─────────────────────────────────────────────────────────────────────────────
...
- Motion physics directly implied by stated actions
...
def rewrite_caption(original: str, backend: str, model: str,
    ...
    """v7e-lite 改写：3处手术式修改（opening + visual inference + camera closing）"""
    ...
    for attempt in range(max_retries + 1):
        ...
        if result_words < 70:
            logger.warning(f"  [重试 {attempt+1}] 输出过短: {result_words} 词 (最低 70)")
            continue
        if result_words > 160:
...
def validate_rewrite(original: str, rewritten: str) -> dict:
    ...
    if new_words < 70:
        issues.append(f"too short ({new_words} words, min 70)")
    if new_words > 160:
...
def main():
    parser = argparse.ArgumentParser(
        description="Hybrid Prompt 改写 v7e-lite：外观优化 + 运动保留（为 SVD 留空间）"
    )
    ...
    parser.add_argument("--temperature", type=float, default=0.4,
                        help="LLM 生成温度 (默认: 0.4, 更低温度确保运动描述不被改动)")
    ...
    logger.info(f"{'='*60}")
    logger.info(f"v7e-lite: 外观优化 + 运动保留（为 SVD 留空间）")
    logger.info(f"{'='*60}")
    ...
    log_data = {
        "version": "v7e_lite",
        "strategy": "surgical_3_modifications_subject_first_visual_inference_camera_closing_motion_preserved",
        "description": "外观优化 + 运动描述完全保留VLM原文，为SVD先验留出运动信息补充空间",
        "backend": args.backend,
        "model": args.model,
        "temperature": args.temperature,
